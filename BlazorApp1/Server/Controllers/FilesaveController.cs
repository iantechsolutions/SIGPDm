...
public class FilesaveController : ControllerBase
{
    ...
    [HttpPost("imagenesRecepciones")]
    public async Task<ActionResult<IList<UploadResult>>> PostFileToImagenesRecepciones([FromForm] IEnumerable<IFormFile> files)
    {
        var maxAllowedFiles = 50;
        long maxFileSize = 1024 * 1500000;
        var filesProcessed = 0;
        ...
        foreach (var file in files)
        {
            ...
            if (filesProcessed < maxAllowedFiles)
            {
                ...
                else
                {
                    ...
                    {
                        trustedFileNameForFileStorage = Path.GetRandomFileName();
                        var path = Path.Combine(env.ContentRootPath, "wwwroot", "imagenesRecepciones", untrustedFileName);
    ...
    [HttpPost("imagenesRecepciones/organigrama")]
    public async Task<ActionResult<IList<UploadResult>>> PostFileToImagenesorganigrama([FromForm] IEnumerable<IFormFile> files)
    {
        var maxAllowedFiles = 50;
        long maxFileSize = 1024 * 1500000;
        var filesProcessed = 0;
        ...
        foreach (var file in files)
        {
            ...
            if (filesProcessed < maxAllowedFiles)
            {
                ...
                else
                {
                    ...
                    {
                        trustedFileNameForFileStorage = Path.GetRandomFileName();
                        var path = Path.Combine(env.ContentRootPath, "wwwroot", "imagenesRecepciones", "organigrama",untrustedFileName);
    ...
    public IActionResult DeleteImage()
    {
        try
        {
            ...
            return NotFound(new { message = "La imagen no existe" });
        }
        catch (Exception ex)
        {
            return StatusCode(500, new { message = "Hubo un error al eliminar la imagen", error = ex.Message });
        }
    }
